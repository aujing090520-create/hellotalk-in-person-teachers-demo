const teachers = [
  ...
  { id: 'yuki', name: 'Yuki', photo: 'p5', country: 'JP', cityCourseCount: { 深圳: 2, 上海: 1, 东京: 1 }, cityCourses: { 深圳: [{ venue: '华侨城创意园', area: '南山区', slots: '本周日 10:00' }, { venue: '科兴科学园共享空间', area: '南山区', slots: '周六 14:00' }, { venue: '南头古城阅文社', area: '南山区', slots: '今天 19:00' }], 上海: [{ venue: '静安语言空间', area: '静安区', slots: '周六 10:00' }, { venue: '徐汇会话馆', area: '徐汇区', slots: '今天 18:30' }, { venue: '前滩书屋', area: '浦东新区', slots: '本周日 15:00' }], 东京: [{ venue: '涩谷学习咖啡', area: '涩谷区', slots: '周六 13:00' }, { venue: '新宿语言角', area: '新宿区', slots: '周日 11:00' }] }, weekendCityCourses: { 深圳: [{ venue: '红树林自然教室', area: '南山区', slots: '周日 16:00' }, { venue: '后海语伴小屋', area: '南山区', slots: '周六 11:00' }, { venue: '华润大厦会客厅', area: '南山区', slots: '今天 20:00' }] } },
...
function renderGlobalCourseList(venues, active) {
  ...
  return `<div class="course-map-list global-course-list">${grouped.map(({ teacher, course, venues: courseVenues }) => `<article class="map-course-card ${course.id === state.selectedCourse && courseVenues.some((venue) => venue.venue === active) ? 'selected' : ''}"><div class="map-course-head"><div><strong>${course.title}</strong><small>${teacher.name} · ${course.duration}</small></div><em>¥${course.price}/节课</em></div><div class="map-course-venues">${courseVenues.map((venue) => `<button data-action="select-global-course-venue" data-course="${course.id}" data-venue="${venue.venue}" class="${course.id === state.selectedCourse && venue.venue === active ? 'selected' : ''}"><b>${venue.venue}</b><small>${venue.sessionTime}</small></button>`).join('')}</div></article>`).join('') || '<div class="empty">没有符合条件的课程</div>'}</div>`;
}
...
function renderCourseVenueBlock(teacher) {
  ...
  return `<div class="course-venue-inline"><span>⌖ 可选地点：${venueLabel}</span><button data-action="open-venue-map" data-id="${teacher.id}" data-venue="${venue.venue}">地图 ›</button></div>`;
}
...
function bindEvents() {
  app.querySelectorAll('[data-action]').forEach((element) => element.addEventListener('click', (event) => {
    ...
    if (action === 'profile') { state.profileId = event.currentTarget.dataset.id; state.profileRoute = 'profile'; state.profileTab = 'archive'; state.profileMore = false; state.selectedCourse = 'conversation'; state.selectedVenue = courseVenues(teacherForProfile())[0]?.venue || ''; }
