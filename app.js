...
function renderProfileTab(teacher) {
  ...
  return `<section class="profile-tab-panel teacher-archive"><section class="teacher-types"><h3>教学类型</h3><div><span>实用口语</span><span>旅行口语</span><span>商务沟通</span></div></section><section class="teacher-self-intro"><h3>老师自我介绍</h3><p>你好，我是 Sarah。在深圳开设一对一中文面授课，擅长把日常场景、旅行和工作沟通带入练习。我们可以先从你的学习目标聊起，再一起约定方便的上课地点。</p><button data-action="translate-bio">文A</button></section><button class="teacher-time-entry" data-action="open-teacher-time"><span>▣</span>查看老师时间</button></section>`;
}
...
function renderProfileVenueDetail(teacher) {
  ...
  return `<div class="profile-venue-detail"><button class="profile-venue-anchor" data-action="profile-venue-next" aria-label="切换上课地点，当前为${venue.venue}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20s6-5.1 6-11a6 6 0 1 0-12 0c0 5.9 6 11 6 11Z"></path><circle cx="12" cy="9" r="2"></circle></svg><small>上课地点 ${index + 1}/${total}</small><strong>${venue.venue}</strong><em>${venue.area} · ${venue.slots} 可约</em></button></div>`;
}
