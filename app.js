...
function renderProfile() {
  ...
  return `<div class="screen ht-profile map-profile"><main class="profile-scroll">${renderProfileVenueHero(teacher)}
    <section class="map-profile-summary"><div class="map-profile-avatar teacher-credential-frame"><span class="profile-avatar-photo photo-${teacher.photo}"></span><svg class="teacher-credential-ring" viewBox="0 0 100 100" aria-hidden="true"><circle class="credential-ring-cyan" cx="50" cy="50" r="48"></circle><circle class="credential-ring-violet" cx="50" cy="50" r="39"></circle><defs><path id="teacher-credential-text" d="M50,5 a45,45 0 1,1 -0.1,0" /></defs><text><textPath href="#teacher-credential-text" startOffset="2%">CERTIFIED TEACHER · CERTIFIED TEACHER · </textPath></text></svg><i class="teacher-cap" aria-hidden="true"><svg viewBox="0 0 24 24"><path class="teacher-cap-top" d="m2.3 8.8 9.7-5.1 9.7 5.1-9.7 5.1-9.7-5.1Z"/><path class="teacher-cap-base" d="M6.2 11.3v3.35c3.25 1.9 8.35 1.9 11.6 0v-3.35"/><path class="teacher-cap-tassel" d="M20.1 9.9v5.7c0 1 .63 1.75 1.45 2.04"/><circle class="teacher-cap-tassel-dot" cx="21.8" cy="18.2" r="1.35"/></svg></i><i class="profile-country-flag" aria-label="${teacher.country}">${flag(teacher.country)}</i></div><div class="map-profile-name"><h1>${teacher.name} <span>♀24</span></h1><div class="map-profile-meta"><p class="map-profile-teaching">教学：中文</p><p class="map-profile-teacher"><i>✦</i>平台老师</p></div></div>${renderTeacherTrustStats()}<i class="teacher-companion-watermark" aria-hidden="true"></i></section>${renderTeacherOverview(teacher)}
...
function bindEvents() {
  app.querySelectorAll('[data-action]').forEach((element) => element.addEventListener('click', (event) => {
    ...
    if (action === 'profile') { state.profileId = event.currentTarget.dataset.id; state.profileRoute = 'profile'; state.profileTab = 'archive'; state.profileVenueIndex = 0; state.profileMore = false; state.sheet = false; state.selectedCourse = 'conversation'; state.selectedVenue = courseVenues(teacherForProfile())[0]?.venue || ''; }
